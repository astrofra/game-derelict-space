...
{		
	...
	function	ClickUpEvent(event, table)
	{
		...
		if(pos_click_down && pos_click_down.Dist2(Vector(g_cursor._x * 1280.0, g_cursor._y * 960.0, 0.0) - pos) <= 0.0)
			CallCallback()
	...
	{	
		...
		else
		{
			...
			if(!g_WindowsManager.folded_icon_horizontal_sizer)
			{
				...
				SpriteShow(g_WindowsManager.folded_icon_horizontal_sizer.array_sprite["little_shadow_sprite"], false)				
			}
	...
	function Update(_with_all_children = false)
	{
		...
		{	
			...
			foreach(id, sprite in array_sprite)
			{
				...
				{
					first_is_shown = SpriteIsShown(sprite)
					break
				}
			}
