...
{		
	...
	function RefreshValueText(_force= false)
	{
		...
		if ("distance_factor" in user_param)
			current_value_string = (user_param["distance_factor"] * current_value).tointeger() + " m"
		else
			current_value_string = current_value

		if(current_value_string.tostring().len() != prev_current_value_string.tostring().len())
		{ 
			local new_rect = TextComputeRect(Rect(0,0,9999,9999), current_value_string.tostring(), "simu_font", { size = g_size_font_text })
		...
		PictureTextRender(array_picture["text_value_part"], Rect(0, 10, SpriteGetSize(array_sprite["text_value_part"]).x, SpriteGetSize(array_sprite["text_value_part"]).y), current_value_string.tostring(), "simu_font" ,{ size = g_size_font_text, color = g_size_font_color_text })		
	...
	{		
		...
		local width_text = 	(TextComputeRect(Rect(0,0,9999,9999), text.tostring(), "simu_font", { size = g_size_font_text }).ex*1.5).tointeger()
		local width_text_part = (TextComputeRect(Rect(0,0,9999,9999), (max_value+step).tostring(), "simu_font", { size = g_size_font_text }).ex*1.5).tointeger()
		...
		// blit text part
		{
			...
			PictureTextRender(pict, Rect(0, 10, width_text, TempRect.ey), text.tostring(), "simu_font" ,{ size = g_size_font_text, color = g_size_font_color_text })
		...
		// blit text value part
		{
			...
			PictureTextRender(pict, Rect(0, 10, width_text_part, TempRect.ey), current_value.tostring(), "simu_font" ,{ size = g_size_font_text, color = g_size_font_color_text })
