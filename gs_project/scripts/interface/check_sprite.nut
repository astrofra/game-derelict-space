...
{		
	...
	function RefreshValueText(_checked, _force=false)
	{
		if(is_shown && (_checked != checked || _force))
		{
			...
			if("text_part" in array_sprite_relative_pos)
			{
				...
				PictureTextRender(array_picture["text_part"], Rect(0, 10, SpriteGetSize(array_sprite["text_part"]).x, SpriteGetSize(array_sprite["text_part"]).y),  text.tostring(), "simu_font" ,{ size = g_size_font_text, color = g_size_font_color_text })					
	...
	{				
		...
		if(_max_width != -1)
			MiddleRect.ex = _max_width - (PictureGetRect(left_picture).ex + PictureGetRect(right_picture).ex + PictureGetRect(check_picture).ex)
		...
		width = PictureGetRect(left_picture).ex + text_rect.ex+ PictureGetRect(check_picture).ex +PictureGetRect(right_picture).ex
		...
		// blit check part
		{
			...
			TextureSetWrapping(texture, false,false)
			local relative_pos = Vector(PictureGetRect(left_picture).ex ,5,0)
		...
		{			
			...
			TextureUpdate(texture, pict)
			TextureSetWrapping(texture, false,false)
			local relative_pos = Vector(PictureGetRect(left_picture).ex + PictureGetRect(check_picture).ex,0,0)
		...
		// add click check section
		{
			...
			TextureSetWrapping(texture, false,false)
			local relative_pos = Vector(PictureGetRect(left_picture).ex,0,0)
			array_texture.rawset("click_check_part", texture)
			local sprite = UIAddSprite(g_WindowsManager.current_ui, -1, texture, pos.x+relative_pos.x, pos.y+relative_pos.y, PictureGetRect(check_picture).ex, height)
