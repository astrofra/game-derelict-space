...
{		
	...
	function RefreshValueText(_force = false)
	{
		...
		PictureTextRender(array_picture["value_part"], Rect(0, 10, SpriteGetSize(array_sprite["value_part"]).x, SpriteGetSize(array_sprite["value_part"]).y), array_value[current_index].tostring()+" "+unit.tostring(), "simu_font" ,{ size = g_size_font_text, color = g_size_font_color_text })					
	...
	{				
		...
		local prev_picture = PictureNew()
		PictureLoadContent(prev_picture, g_skin[skin].prev_picture)
		local next_picture = PictureNew()
		PictureLoadContent(next_picture, g_skin[skin].next_picture)
		
		local prev_next_rectangle_width = 0
		foreach(value in _array_value)
		{
			local width_value = TextComputeRect(Rect(0,0,9999,9999), value.tostring()+" "+unit.tostring(), "simu_font", { size = g_size_font_text }).ex
			if(width_value > prev_next_rectangle_width)
				prev_next_rectangle_width = width_value
		}

		local text_rect = TextComputeRect(Rect(0,0,9999,9999), _text, "simu_font", { size = g_size_font_text })

		// get the rect occupied by the text
		local MiddleRect = Rect(0,0,9999,9999)
		MiddleRect.ex = text_rect.ex + PictureGetRect(prev_picture).ex + prev_next_rectangle_width + PictureGetRect(next_picture).ex
		MiddleRect.ey = PictureGetRect(middle_picture).ey

		if(_max_width != -1)
			MiddleRect.ex = _max_width - (PictureGetRect(left_picture).ex + PictureGetRect(right_picture).ex + PictureGetRect(prev_picture).ex + PictureGetRect(next_picture).ex)
		...
		width = PictureGetRect(left_picture).ex + text_rect.ex+ PictureGetRect(prev_picture).ex + prev_next_rectangle_width + PictureGetRect(next_picture).ex + PictureGetRect(right_picture).ex 
		...
		// blit text part
		{
			...
			PictureTextRender(pict, Rect(0, 10, text_rect.ex, MiddleRect.ey), _text, "simu_font" ,{ size = g_size_font_text, color = g_size_font_color_text })
		...
		// blit prev part
		{
			...
			TextureUpdate(texture, prev_picture)
			TextureSetWrapping(texture, false,false)
			local relative_pos = Vector(PictureGetRect(left_picture).ex + text_rect.ex,0,0)
			array_texture.rawset("prev_part", texture)
			array_sprite.rawset("prev_part", UIAddSprite(g_WindowsManager.current_ui, -1, texture, pos.x+relative_pos.x, pos.y+relative_pos.y, text_rect.ex , MiddleRect.ey))
		...
		// blit value part
		{
			...
			PictureTextRender(pict, Rect(0, 10, prev_next_rectangle_width, MiddleRect.ey), _array_value[current_index].tostring()+" "+unit.tostring(), "simu_font" ,{ size = g_size_font_text, color = g_size_font_color_text })
			
			TextureUpdate(texture, pict)
			TextureSetWrapping(texture, false,false)
			local relative_pos = Vector(PictureGetRect(left_picture).ex + text_rect.ex + PictureGetRect(prev_picture).ex,0,0)
		...
		// blit next part
		{
			...
			TextureUpdate(texture, next_picture)
			TextureSetWrapping(texture, false,false)
			local relative_pos = Vector(PictureGetRect(left_picture).ex + text_rect.ex + PictureGetRect(prev_picture).ex + prev_next_rectangle_width,0,0)
			array_texture.rawset("next_part", texture)
			array_sprite.rawset("next_part", UIAddSprite(g_WindowsManager.current_ui, -1, texture, pos.x+relative_pos.x, pos.y+relative_pos.y, text_rect.ex , MiddleRect.ey))
		...
		// add click prev section
		{
			...
			TextureSetWrapping(texture, false,false)
			local relative_pos = Vector(PictureGetRect(left_picture).ex + text_rect.ex,0,0)
			array_texture.rawset("click_prev_part", texture)
			local sprite = UIAddSprite(g_WindowsManager.current_ui, -1, texture, pos.x+relative_pos.x, pos.y+relative_pos.y, PictureGetRect(prev_picture).ex, height)
		...
		// add click next section
		{
			...
			TextureSetWrapping(texture, false,false)
			local relative_pos = Vector(PictureGetRect(left_picture).ex + text_rect.ex + PictureGetRect(prev_picture).ex + prev_next_rectangle_width,0,0)
			array_texture.rawset("click_next_part", texture)
			local sprite = UIAddSprite(g_WindowsManager.current_ui, -1, texture, pos.x+relative_pos.x, pos.y+relative_pos.y, PictureGetRect(next_picture).ex, height)
