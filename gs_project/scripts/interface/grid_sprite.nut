...
{		
	...
	function	CursorMoveSliderEvent(event, table)
	{
		...
		{			
			...
			start_id = ((child_array.len()/max_sprite_H - (skip_scroll_line+max_sprite_V)) * percent_scrollbar).tointeger()
			if(start_id < 0)
				start_id = 0
	...
	function Show(_with_all_children = false)
	{
		...
		if(child_array.len() <= (max_sprite_V+ skip_scroll_line)*max_sprite_H )
			SpriteShow(array_sprite["list_scrollbar"], false)
		else
			SpriteShow(array_sprite["list_scrollbar"], true)
	}	
		...
		if(width_total_child > 0)
		{
			for(local column=0; column < max_sprite_H; ++column)
			{
				...
				if(show_column_array[column])
					for(local child_id = column; child_id < child_array.len(); child_id += max_sprite_H)
					{
							child_array[child_id].SetSize((size_column[column] * width.tofloat()) / width_total_child.tofloat(), height)
					}	
			}		
...
//		SetSize(width, height)
	...
	{			
		...
		{
			local pict_list_scrollbar = PictureNew()
			PictureLoadContent(pict_list_scrollbar, g_skin[skin].vertical_scroll_picture)
			local texture = ResourceFactoryNewTexture(g_factory)		
			TextureUpdate(texture, pict_list_scrollbar)
			TextureSetWrapping(texture, false,false)
			array_sprite.rawset("list_scrollbar", UIAddSprite(g_WindowsManager.current_ui, -1, texture, pos.x + width -PictureGetRect(pict_list_scrollbar).ex, pos.y, PictureGetRect(pict_list_scrollbar).ex, PictureGetRect(pict_list_scrollbar).ey))
			SpriteShow(array_sprite["list_scrollbar"], false)
